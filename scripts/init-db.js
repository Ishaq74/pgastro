...
/**
 * Initialiseur de base de données pour Better Auth (version JS)
 ...
 */
async function checkTablesExist(db, isPostgreSQL) {
  try {
    if (isPostgreSQL) {
 ...
 */
async function createTablesIfNotExist(db, isPostgreSQL) {
  try {
 ...
 */
async function initializeBetterAuthDatabase() {
  ...
  try {
    ...
    if (!isPostgreSQL && typeof db.close === 'function') {
      db.close();
    } else if (isPostgreSQL && typeof db.end === 'function') {
      await db.end();
    }
  }
}
